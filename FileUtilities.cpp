...
const int DIRECTORY_NAME_LENGTH = 255;
const int COPY_BUFFER_SIZE      = 409600;
const int MAX_TMP_TRYING_TIME   = 50000;

using namespace std;

namespace FileUtilities {
// checks if a file exists, exits otherwise
bool CheckFile(const char* filename, bool showError) {
...
//void CheckDirectory(const string& directory) {
//
...
void SearchDirectory(vector<string>& filenames, const char* directory) {
	...
	if(directory[directoryNameLen - 1] != OS_DIRECTORY_SEPARATOR) 
		sprintf_s(tDirectory, DIRECTORY_NAME_LENGTH * sizeof(for_size), "%s%c", directory, OS_DIRECTORY_SEPARATOR);
	else 
		sprintf_s(tDirectory, DIRECTORY_NAME_LENGTH * sizeof(for_size), "%s", directory);
